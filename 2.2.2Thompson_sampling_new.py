...
block_mu = 5         # 块选择策略中成功块的个数(老师的要求这个最终可能要设置成1 ?)
max_iter = 10      # 增加迭代次数以观察趋势,这个不应该设的太小(比如3)
num_test_classes = 200  # 测试类别数（从200类中随机选择）
lambda_sparsity = 0.3
debug_mode = 0
每个类采样10%样本
实验结果：40轮左右就降到45.50%左右，攻击成功率达到51.7%.之后长时间在这个范围附近波动
60轮降到44.70%，攻击成功率52.5%
最终打印：
🔥GPU攻击进程: 100%|█| 10/10 [00:29<00:00,  2.91s/it, Current_Acc=39.20%, Global_Acc=39.08%, Loss=-26.1747]
🔥GPU攻击进程: 100%|█| 10/10 [00:29<00:00,  2.95s/it, Current_Acc=39.60%, Global_Acc=39.33%, Loss=-22.7905]
攻击进度: 100%|██| 200/200 [2:44:28<00:00, 49.34s/it, Success=49.5%]

最终攻击成功率: 49.5%(这个为什么比之前低了一点？能不能再测一下？)
平均L0扰动: 17.4, 平均L1扰动: 7.8
(1.1)——(1)的重复实验：
