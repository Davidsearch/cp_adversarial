...
'''
单运行该程序nvidia-smi情况:
42W /  70W |   1787MiB / 15360MiB |      0%      Default
准确率下降的速度相比MAB慢很多。第74个类还在44.40%!而且波动好几轮都不下降!
class 80 :43.50%; class 127 41.50%
🔥GPU攻击进程: 100%|█| 10/10 [01:36<00:00,  9.65s/it, Current_Acc=39.40%, Global_Acc=39.49%, Loss=-25.3712]
攻击进度: 100%|█| 199/200 [6:51:29<03:03, 183.53s/it, Success=49.2%]GMM初始化成功
🔥GPU攻击进程: 100%|| 10/10 [01:54<00:00, 11.43s/it, Current_Acc=39.50%, Global_Acc=39.42%, Loss=-22.8623]
攻击进度: 100%|█| 200/200 [6:54:26<00:00, 124.33s/it, Success=49.5%]

最终攻击成功率: 49.5%
平均L0扰动: 17.2, 平均L1扰动: 7.8
...
# 块选择策略
block_size = 8     # 块尺寸
num_samples = 12     # 每轮采样块数
block_mu = 1         # 块选择策略中成功块的个数(老师的要求这个最终可能要设置成1 ?)
max_iter = 20      # 增加迭代次数以观察趋势,这个不应该设的太小(比如3)
...
# ---------------------------- 新增GMM模块 ---------------------------
class GMMBlockSelector:
    """基于GMM-EM的块选择器"""
    
    def __init__(self, total_blocks, block_size, img_size=64, n_components=2):
        ...
        # 特征存储结构
        self.block_features = {
            'success_count': np.zeros(total_blocks),
            'recent_selected': np.zeros(total_blocks),
            'avg_loss_change': np.zeros(total_blocks),
            'perturb_norm': np.zeros(total_blocks)
        }
        
        # GMM模型参数
        self.gmm = GaussianMixture(n_components=n_components, 
                                  reg_covar=1e-6, max_iter=200)
        self.prob = np.ones(total_blocks) / total_blocks

    def extract_features(self, bid):
        """提取单个块的特征向量"""
        rows_per_block = self.img_size // self.block_size
        row = bid // rows_per_block
        col = bid % rows_per_block
        x = row * self.block_size + self.block_size/2
        y = col * self.block_size + self.block_size/2
        
        return np.array([
            x / self.img_size,  # 归一化位置特征
            y / self.img_size,
            self.block_features['success_count'][bid],
            np.log1p(self.block_features['recent_selected'][bid]),
            self.block_features['avg_loss_change'][bid],
            np.log1p(self.block_features['perturb_norm'][bid])
        ])

    def update_features(self, selected_blocks, fitness_values):
        """更新块特征统计"""
        # 更新最近选择次数(注意每个块有可能会被选择多次，所以这里的代码逻辑应该是合理的)
        unique, counts = np.unique(selected_blocks, return_counts=True)
        for bid, cnt in zip(unique, counts):
            self.block_features['recent_selected'][bid] = 0.9 * self.block_features['recent_selected'][bid] + 0.1 * cnt
            
        # 计算成功次数（前50%视为成功）[debug1: selceted_blocks是选出来多少个？是前50%吗？我记得不是]
        median = np.median(fitness_values)
        for bid, fv in zip(selected_blocks, fitness_values):
            if fv > median:
                self.block_features['success_count'][bid] += 1

    def update_perturbation(self, bid, perturbation):
        """更新块的扰动特征"""
        self.block_features['perturb_norm'][bid] = np.linalg.norm(perturbation)
        
    def train_gmm(self):
        """训练GMM模型"""
        features = np.array([self.extract_features(bid) 
        ...
        try:
            self.gmm.fit(features[valid_mask])
            posterior = self.gmm.predict_proba(features)
            self.prob = posterior[:, 0]  # 假设第一个成分是高成功率的块
            self.prob = self.prob / (self.prob.sum() + 1e-8)
        except Exception as e:
            print(f"GMM训练失败: {str(e)}")

    def get_probs(self):
        return torch.from_numpy(self.prob).float()
...
class SZOAttack:
    ...
    def __init__(self, model, image, label, block_size, device, epsilon=0.1,universal_perturbation = None,original_path=None):  # 新增epsilon参数
       ...
       # === 新增批量图像支持 ===
        ...
        # GMMBlockSelecter在这里被初始化为对象
        self.gmm_selector = GMMBlockSelector(
    ...
    def update_prob(self, blocks, fitness_values):
        ...
        # 1. 更新GMM特征
        self.gmm_selector.update_features(blocks_np, fitness_values)
        
        # 2. 更新扰动范数
        for bid in np.unique(blocks_np):
            if bid in self.cma_blocks:
                perturbation = self.cma_blocks[bid].mean
                self.gmm_selector.update_perturbation(bid, perturbation)
                
        # 3. 训练GMM模型
        self.gmm_selector.train_gmm()
        
        # 4. 获取新概率
        self.prob = self.gmm_selector.get_probs().to(self.device)
    ...
    def attack(self, val_loader, device, enable_eval=True, show_progress=True):
        ...
        for t in progress_bar:
            ...
            # print("attack:advw_images", adv_images.shape)
